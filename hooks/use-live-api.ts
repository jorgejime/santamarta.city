...
export function useLiveApi({
 ...
 useEffect(() => {
   ...
   const onSetupComplete = () => {
     ...
     client.sendRealtimeText('hola');
   };

   const onClose = (event: CloseEvent) => {
     setConnected(false);
     stopAudioStreamer();
     let reason = "La sesión ha terminado. Presiona 'Play' para iniciar una nueva sesión. "+ event.reason;
   ...
   const onToolCall = async (toolCall: LiveServerToolCall) => {
     ...
     try {
       ...
       for (const fc of toolCall.functionCalls) {
         ...
         const triggerMessage = `Activando llamada de función: **${
           fc.name
         ...
         try {
           ...
           if (toolImplementation) {
             ...
             toolResponse = `Herramienta desconocida llamada: ${fc.name}.`;
           ...
           const errorMessage = `Error al ejecutar la herramienta ${fc.name}.`;
       ...
       if (functionResponses.length > 0) {
         const responseMessage = `Respuesta de la llamada de función:\n\`\`\`json\n${JSON.stringify(
           functionResponses,
           null,
           2,
         )}\n\`\`\``;
