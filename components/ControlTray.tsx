...
function ControlTray({trayRef}: ControlTrayProps) {
  ...
  const handleTextSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!connected) {
      ...
      useLogStore.getState().addTurn({
        role: 'system',
        text: `No se puede enviar el mensaje. Por favor, conéctate primero.`,
        isFinal: true,
      });
      return;
    }
  ...
  const handleSettingsClick = () => {
    ...
    if (settingsClickTimestamps.current.length >= 6) {
        ...
        useLogStore.getState().addTurn({
            role: 'system',
            text: "¡Has desbloqueado el modo Búsqueda del Tesoro!",
            isFinal: true,
        });
    ...
    <section className="control-tray" ref={trayRef}>
      <nav className={cn('actions-nav', { 'text-entry-visible-landscape': isLandscape && isTextEntryVisible })}>
        ...
        <button
          type="button"
          aria-label={
            !speakerMuted ? 'Salida de audio activada' : 'Salida de audio desactivada'
          }
          ...
          title={!speakerMuted ? 'Silenciar salida de audio' : 'Activar salida de audio'}
        >
        ...
        <button
          ...
          title="Alternar entrada de texto"
        >
          <span className="icon">
        ...
        {(!isMobile || isTextEntryVisible) && (
          <form className="prompt-form" onSubmit={handleTextSubmit}>
            <input
              ...
              placeholder={
                connected ? 'Escribe un mensaje...' : 'Conéctate para empezar a escribir...'
              }
              value={textPrompt}
